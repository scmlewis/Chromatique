...
export default function HarmonyPanel({ baseColor, onApplyPalette, onSaveFavorite, onCopyHex }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-[var(--color-surface-container-low)] p-1.5 rounded-2xl grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-1.5 shadow-md">
        {HARMONY_TYPES.map((t) => (
          <button
            key={t.id}
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-72 flex flex-col gap-4">
          <div className="p-4 bg-[var(--color-surface-container-low)] rounded-2xl shadow-md">
            <span className="text-[10px] font-bold text-[var(--color-primary)] uppercase tracking-widest mb-3 block">Base Color</span>
            <div className="flex flex-col items-center">
              ...
              <div className="mt-2 w-full flex items-center gap-2">
                <div 
                  className="w-10 h-10 md:w-8 md:h-8 rounded-lg shadow-inner flex-shrink-0" 
                ...
                <input 
                  ...
                  inputMode="text"
                  pattern="^#?[0-9A-Fa-f]{6}$"
                  className="flex-1 bg-[var(--color-surface-container-low)] px-3 py-1.5 md:py-1 rounded-lg text-xs font-mono uppercase text-white outline-none focus:bg-[var(--color-surface-container-high)] transition-colors"
                />
                <button 
        ...
        <div className="flex-1 p-6 bg-[var(--color-surface-container-low)] rounded-3xl shadow-lg flex flex-col justify-between">
           <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                {harmonies.map((h, i) => (
                  <div key={i} className="group relative animate-pop" style={{ animationDelay: `${i * 30}ms` }}>
                    <div 
                      className="aspect-square w-full rounded-2xl shadow-lg transition-all group-hover:scale-[1.05] group-hover:shadow-2xl cursor-pointer"
