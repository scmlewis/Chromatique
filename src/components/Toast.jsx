...
export default function Toast({ message, actionLabel, onAction, onClose, previewColors = [], type = 'info' }) {
  return (
    ...
    <div className="toast-container">
      <div className="toast-content animate-pop" style={{ background: 'var(--color-surface-elevated)', border: '1px solid var(--color-border-ghost)', borderRadius: 'var(--radius-lg)' }}>
        ...
        <div className="flex items-start gap-3 sm:gap-4 mb-3 sm:mb-0">
          ...
          <button onClick={onClose} aria-label="Close toast" className="hidden sm:block icon-btn flex-shrink-0">
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:items-center gap-2 sm:gap-3 sm:ml-auto">
          ...
          <button onClick={onClose} aria-label="Close toast" className="sm:hidden icon-btn self-end">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
